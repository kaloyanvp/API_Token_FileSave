...
// API endpoint - променете според вашите нужди 
    const API_ENDPOINT = 'http://localhost:3000/upload/save';
...
// Send to API
sendBtn.addEventListener('click', sendDataToAPI);
...
function sendToAPI() {
    ...
    fetch(API_ENDPOINT, {
        ...
        .catch(error => {
            loadingSpinner.classList.add('hidden');
            showResultAlert(`Грешка при изпращане: ${error.message}`, 'danger');
